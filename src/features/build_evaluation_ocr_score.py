...
def evaluation_ocr_score():
     ...
     for row in train_data.itertuples(index=False):
          ...
          choices_list = solution['choices'].apply(eval).apply(lambda x: x[0] if isinstance(x[0], list) else x).tolist() # list[list[str]]
          answers = solution['answer'].tolist()                    # list[str]

          # print(f"""
          # questions: {type(questions)} | element: {type(questions[0])}
          # choices:   {type(choices_list)} | element: {type(choices_list[0])}, sub-element: {type(choices_list[0][0])}
          # answers:   {type(answers)} | element: {type(answers[0])}
          # """)
          ...
          image_processor.image.save(f"{PROCESSED_IMAGE_DIR}/image_{row.id}_processed.png", format="PNG")
          ...
          image_processor.image.save(f"{RESET_IMAGE_DIR}/image_{row.id}_reset.png", format="PNG")
          ocr_score, num_char = vqa_evaluator.ocr(image_processor.image, free_chars=4, use_num_char=True)
          print("-"*50)
